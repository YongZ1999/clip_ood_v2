...
def main(args):
    ...
    # 构建覆盖全部 10 个数据集的 frozen ZS 分类器（用于评估未见任务）
    logging.info("Building global frozen zero-shot classifier...")
    ...
    metrics_lada = ContinualLearningMetrics(task_names)
    metrics_lada_ens = ContinualLearningMetrics(task_names)
    metrics_rgda = ContinualLearningMetrics(task_names)
    metrics_rgda_ens = ContinualLearningMetrics(task_names)
    ...
    for i, task_datasets in enumerate(args.dataset_sequence):
        ...
        print("\n=== Evaluating Task ===")
        step_zs, step_lada, step_lada_ens = {}, {}, {}
        step_rgda, step_rgda_ens = {}, {}

        eval_label_offset = 0
        K = len(args.dataset_sequence)
        for j in range(K):
            ...
            zs_acc, lada_acc, lada_ens_acc, rgda_acc, rgda_ens_acc, c_len = \
                evaluate_dataset_lada(
                    args, d_name, model, frozen_zeroshot_classifier, trainer.lada_classifier,
                    lr_rgda_classifier, current_num_classes, eval_label_offset,
            ...
            step_zs[d_name] = zs_acc / 100.0
            step_lada[d_name] = lada_acc / 100.0
            step_lada_ens[d_name] = lada_ens_acc / 100.0
            step_rgda[d_name] = rgda_acc / 100.0
            step_rgda_ens[d_name] = rgda_ens_acc / 100.0

        metrics_zs.update(i, step_zs)
        metrics_lada.update(i, step_lada)
        metrics_lada_ens.update(i, step_lada_ens)
        metrics_rgda.update(i, step_rgda)
        metrics_rgda_ens.update(i, step_rgda_ens)
    ...
    metrics_zs.print_summary()
    metrics_lada.print_summary()
    metrics_lada_ens.print_summary()
    metrics_rgda.print_summary()
    metrics_rgda_ens.print_summary()
    ...
    save_results = {
        ...
        "accuracy_matrix": metrics_lada_ens.get_accuracy_matrix().tolist(),
        "metrics": {
            "zero_shot": metrics_zs.get_summary(),
            "lada": metrics_lada.get_summary(),
            "lada_ensemble": metrics_lada_ens.get_summary(),
            "lr_rgda": metrics_rgda.get_summary(),
            "lr_rgda_ensemble": metrics_rgda_ens.get_summary(),
        }
    }
