...
\title{Null-Space Parameterized Low-Rank Adaptation\\
with Statistical Replay Classifiers\\
for Scalable CLIP Continual Learning}
...
\begin{abstract}
Continual learning with vision-language models like CLIP offers a pathway toward scalable machine learning systems by leveraging transferable representations. However, adapting CLIP to downstream tasks while preserving its zero-shot transfer ability remains challenging. Existing methods primarily focus on either the training phase or the inference phase independently, overlooking the potential synergies between representation adaptation and classifier construction. In this paper, we study a two-stage framework for CLIP continual learning. At the training stage, we introduce LoRA-NSP (Low-Rank Adaptation with Null Space Parameterization), establishing a theoretical connection between null space projection (NSP) and low-rank adaptation (LoRA). We show that NSP can be viewed as a restricted form of LoRA with limited plasticity, and propose a soft forward-pass projection mechanism that preserves all eigendirections while biasing updates away from high-energy historical subspaces. At the inference stage, we study LR-RGDA (Low-Rank Regularized Gaussian Discriminant Analysis), a regularized Gaussian discriminant classifier that uses low-rank covariance structure and the Woodbury matrix identity to reduce classifier cost. We further combine LR-RGDA with the zero-shot classifier and evaluate when this inference-time classifier is competitive with label-memory baselines such as LADA. Our current evidence suggests that the most promising regime is lightweight statistical replay, where historical real features are replaced by compact class statistics rather than stored explicitly.
\end{abstract}
