...
\title{Null-Space Parameterized Low-Rank Adaptation with Bayesian Ensemble Classifiers for Scalable CLIP Continual Learning}
...
\texttt{\{raoxuan88, zhaobo\}@bnu.edu.cn}, \texttt{12532847@mail.sustech.edu.cn}, \texttt{derong@ahu.edu.cn}, \texttt{cesare.alippi@usi.ch}
}
...
\begin{abstract}
Continual learning with vision-language models like CLIP offers a pathway toward scalable machine learning systems by leveraging its transferable representations. However, adapting CLIP to downstream tasks while preserving its zero-shot transfer ability remains a significant challenge. Existing methods primarily focus on either the training phase or the inference phase independently, overlooking the potential synergies between them. In this paper, we propose a holistic framework that co-optimizes both training and inference stages for CLIP continual learning through theoretically grounded mechanisms. At the training stage, we introduce LoRA-NSP (Low-Rank Adaptation with Null Space Parameterization), establishing a novel theoretical connection between null space projection (NSP) and low-rank adaptation (LoRA). We prove that NSP represents a degenerate form of LoRA with limited plasticity, and propose a soft forward-pass projection mechanism that generalizes both approaches while preserving the full expressivity of LoRA. At the inference stage, we propose LR-RGDA (Low-Rank Regularized Gaussian Discriminant Analysis), a Bayes-optimal classifier that achieves computational efficiency through Woodbury matrix identity-based low-rank factorization, reducing complexity from $\mathcal{O}(Cd^2)$ to $\mathcal{O}(Cd + Cr^2d)$. We further introduce an ensemble classifier framework that combines LR-RGDA with the zero-shot classifier, achieving principled balance between in-distribution and out-of-distribution classification. Our approach achieves state-of-the-art performance on X-TAIL benchmark, demonstrating the effectiveness of our co-optimization strategy.
\end{abstract}
...
To address these limitations, we propose a holistic framework that co-optimizes both the training and inference stages of CLIP continual learning.
...
\textbf{Inference Stage: Ensemble Classifier.} At the inference stage, we propose an \textbf{ensemble classifier framework} that integrates LR-RGDA (Low-Rank Regularized Gaussian Discriminant Analysis) with the zero-shot classifier. LR-RGDA provides principled uncertainty estimation that naturally discriminates between ID and OOD samples, enabling reliable routing without additional training.

%------------------------------------------------------------------------------
% Paragraph 4: Contributions
Our contributions are summarized as follows:
\begin{itemize}
    \item We establish a novel theoretical connection between null space projection (NSP) and low-rank adaptation (LoRA), proving that NSP represents a degenerate form of LoRA with limited plasticity. Building on this insight, we propose LoRA-NSP with soft forward-pass projection for better stability-plasticity trade-offs, and design an FD+CD distillation mechanism to maintain representation stability and vision-language alignment during continual learning.
    \item We develop LR-RGDA, a Bayes-optimal classifier that exploits low-rank covariance structures via the Woodbury matrix identity, reducing inference complexity from $\mathcal{O}(Cd^2)$ to $\mathcal{O}(Cd + Crd)$ while maintaining the expressivity of quadratic discriminant analysis.
    \item We introduce an ensemble classifier framework that combines LR-RGDA with zero-shot classification via an adaptive router, achieving strong OOD detection and competitive ID classification through principled integration of supervised and zero-shot classifiers.
\end{itemize}

%------------------------------------------------------------------------------
% Paragraph 5: Experiments
We evaluate our method on the recently proposed X-TAIL benchmark~\cite{xu2024rail}, which consists of 10 image classification datasets from diverse domains. \todo{[Experiments Summary] Our ablation studies confirm that each component (NSP, FD, CD) contributes meaningfully. Fill with actual numbers and findings upon completing experiments.}
...
\textbf{Motivation.}
After fine-tuning, the zero-shot classifier leverages CLIP's pre-trained vision-language alignment for classification. However, it relies solely on visual-text similarity, missing discriminative information captured during fine-tuning. We propose an ensemble classifier that combines a Low-Rank Regularized Gaussian Discriminant Analysis (LR-RGDA) classifier with the zero-shot classifier. Our approach is theoretically grounded in Bayesian decision theory and achieves computational efficiency through low-rank factorization.

\textbf{Regularized Gaussian Discriminant Analysis.}
Building upon Gaussian statistics, we aim to construct a classifier that approximates the Bayes-optimal decision rule. After training on task $t$, we model the deep features of each class $c \in \mathcal{C}_t$ as a multivariate Gaussian distribution $p(\mathbf{z}|c) = \mathcal{N}(\boldsymbol{\mu}_c, \mathbf{\Sigma}_c)$, where $\mathbf{z} = f_\theta(\mathbf{x})$ is the feature representation and $\mathcal{C}_t$ is the set of classes observed up to task $t$.
...
\begin{equation}
    ...
    \label{eq:rgda}
\end{equation}
The RGDA classifier selects the class with maximum discriminant score: $\hat{y} = \arg\max_{c \in \mathcal{C}_t} g_c^{\text{RGDA}}(\mathbf{z})$. A formal Bayes-optimality guarantee is provided in Appendix~\ref{app:bayes}.

\textbf{Low-Rank Factorized RGDA.}
While RGDA enjoys Bayes optimality under Gaussian assumptions, its inference complexity scales as $\mathcal{O}(Cd^2)$ due to per-class matrix-vector products with $(\mathbf{\Sigma}_c^{\text{reg}})^{-1}$. This becomes prohibitive when both the number of classes $C$ and the feature dimension $d$ are large.
...
\textbf{OOD Detection Property.}
A key property of LR-RGDA is that it outputs positive confidence only for in-distribution (ID) samples while assigning near-zero confidence to out-of-distribution (OOD) samples. This stems from its supervised nature: the classifier is trained exclusively on ID data, so OOD samples fall outside the support of the learned Gaussian distributions, producing near-zero posterior probabilities. This provides a natural ID/OOD discriminator without requiring additional OOD detection training.

\textbf{Ensemble Framework.}
The ensemble classifier balances between discriminative ID classification (LR-RGDA) and generalized zero-shot capability through a contribution coefficient $\alpha$:
\begin{equation}
    p_{\text{ens}}(y|\mathbf{x}) = \alpha \cdot p_{\text{LR-RGDA}}(y|\mathbf{x}) + (1-\alpha) \cdot p_{\text{ZS}}(y|\mathbf{x})
\end{equation}
for ID classes, and $p_{\text{ens}}(y|\mathbf{x}) = p_{\text{ZS}}(y|\mathbf{x})$ for OOD classes. The adaptive router further improves OOD detection by routing samples with LR-RGDA confidence below threshold $\tau$ to the zero-shot classifier, ensuring robust classification across both seen and unseen domains.
...
We evaluate the OOD detection capability of our LR-RGDA-based detector. Table~\ref{tab:ood} reports the AUROC and FPR@95TPR metrics on X-TAIL. Our method achieves strong OOD detection performance, demonstrating the effectiveness of using LR-RGDA confidence for ID/OOD discrimination.
...
In this paper, we propose a holistic framework for CLIP continual learning that co-optimizes both training and inference stages. At the training stage, LoRA-NSP integrates null space constraints with low-rank adaptation to mitigate catastrophic forgetting while enabling efficient task-specific adaptation. At the inference stage, our ensemble classifier framework combines LR-RGDA with zero-shot classification to achieve optimal balance between ID and OOD classification. Extensive experiments on the X-TAIL benchmark validate the effectiveness of our co-optimization strategy. \todo{[Conclusion] Revise final sentence based on actual results: replace "state-of-the-art performance" with quantitative statement if supported by data.}
...
\section{Bayes Optimality of RGDA}
\label{app:bayes}

\begin{theorem}[Bayes Optimality of RGDA]
\label{thm:bayes}
...
\begin{align}
    ...
    &\quad - (\mathbf{z}^\top \mathbf{B}^{-1} \mathbf{U}_c \mathbf{M}_c^{-1} \mathbf{U}_c^\top \mathbf{B}^{-1} \mathbf{z} - 2\boldsymbol{\mu}_c^\top \mathbf{B}^{-1} \mathbf{U}_c \mathbf{M}_c^{-1} \mathbf{U}_c^\top \mathbf{B}^{-1} \mathbf{z} + \boldsymbol{\mu}_c^\top \mathbf{B}^{-1} \mathbf{U}_c \mathbf{M}_c^{-1} \mathbf{U}_c^\top \mathbf{B}^{-1} \boldsymbol{\mu}_c). \label{eq:quad_expand}
\end{align}
